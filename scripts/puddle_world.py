...
class Puddle:
    ...
    def draw(self, ax, elems):
        ...
        r = patches.Rectangle(self.lowerleft, w, h,
                              color="blue", alpha=self.depth)
...
class PuddleIgnoreAgent(EstimationAgent):
    ...
    def policy(cls, pose, goal):
        ...
        direction = int((math.atan2(dy, dx) - theta) *
                        180/math.pi)
    ...
    def draw(self, ax, elems):
        ...
        elems.append(ax.text(x+1.0, y-0.5, "reward/sec:" +
                     str(self.reward_per_sec()), fontsize=8))
        elems.append(ax.text(
            x+1.0, y-1.0, "eval: {:.1f}".format(self.total_reward+self.final_value), fontsize=8))
